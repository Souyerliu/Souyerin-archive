...
- **五色定理**：任意平面图都可以被五种颜色染色。
  ...
  - 接下来我们对顶点数$v$进行归纳证明：
    ...
    - 下面我们讨论$u$的度数恰好为$5$的情况。假设它的五个相邻点为$a,b,c,d,e$（按顺时针标记），分别被染成颜色$c_1,c_2,c_3,c_4,c_5$。  
      ...
      如果$a$和$c$之间存在一条只由$c_1$和$c_3$构成的路径，且$b$和$d$之间也存在一条只由$c_2$和$c_4$构成的路径，那么由平面性可知这两条路径必然在一个顶点相交（如下图所示），这导致这个顶点的颜色无法确定（$\{c_1,c_3\}\cap\{c_2,c_4\}=\varnothing$），故不会产生这种情况。
