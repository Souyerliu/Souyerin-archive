...
function SidebarContents(props: SidebarContentsProps) {
  ...
  const toc = () => props.toc ?? [];
  const postBaseUrl = () => (props.currentSlug ? `/posts/${props.currentSlug}/` : "");
  ...
  const handleTocClick = (event: MouseEvent, id: string, index: number) => {
    ...
    if (target) {
      ...
      // 保留当前文章路径，避免点击目录后 URL 只剩下孤立的 hash
      const baseUrl = postBaseUrl();
      if (baseUrl) {
  ...
  const handleChildTocClick = (event: MouseEvent, id: string) => {
    event.preventDefault();
    const target = document.getElementById(id);
    if (target) {
      target.scrollIntoView({ behavior: "smooth" });
            ...
            {(item, i) => (
              ...
              >
                <a
                  href={`#${item.id}`}
                  class="toc-link"
                  // Native listener: run before Astro ClientRouter's delegated document listener.
                  // Otherwise the router performs an instant hash jump before smooth scrolling starts.
                  on:click={(e) => handleTocClick(e, item.id, i())}
                >
                  {item.text}
                </a>
                      ...
                      {(child) => (
                        <li class="toc-item">
                          <a
                            href={`#${child.id}`}
                            class="toc-link"
                            on:click={(e) => handleChildTocClick(e, child.id)}
                          >
                            {child.text}
                          </a>
                        </li>
