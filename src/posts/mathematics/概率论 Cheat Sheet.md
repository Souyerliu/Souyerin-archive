...
- 其他性质：
  ...
  - 若$A\cap B=\varnothing$，则$P(A|B)=\frac{P(A)}{P(B)}$
- 乘法公式：
...
- 定义：
  - 设离散型随机变量$X$有分布列$P(X=x_k)=p_k,k=1,2,\cdots$，若
    ...
    $$
    绝对收敛，则称其为$X$的数学期望，记为$\mathbb{E}X$；
  - 设连续型随机变量$X$有概率密度函数$p(x)$，若
    $$
    \int_{-\infty}^\infty xp(x)dx
    $$
    绝对收敛，则称其为$X$的数学期望，也记为$\mathbb{E}X$；

- 随机变量函数的期望：当$Y=f(X)$为随机变量$X$的函数，若$\mathbb{E}(f(X))$存在，则
  $$
...
- 性质：
  - $\mathbb{E}(c)=c$（$c$为常数）
  - $\mathbb{E}(aX)=a\mathbb{E}X$（$a$为常数）
  - $\mathbb{E}(f(X)+g(X))=\mathbb{E}(f(X))+\mathbb{E}(g(X))$

### 方差

- 定义：若$\mathbb{E}(X-\mathbb{E}X)^2$存在，则称其为随机变量$X$的方差，记为$\mathrm{Var}X$
...
- 性质：
  ...
  - $\mathrm{Var}X\geq 0$
- 常见分布期望&方差汇总：
  |分布|形式|期望$\mathbb{E}X$|方差$\mathrm{Var}X$|
  |:---:|:---:|:---:|:---:|
...
- 设随机变量$X\geq 0$且$\mathbb{E}X$存在，则$\forall\epsilon>0$，有
  $$
...
- 设$k$为正整数，若$X^k$数学期望存在，则称$\mathbb{E}(X-\mathbb{E}X)^k$为$X$的$k$阶**中心矩**（记为$\nu_k$），称$\mathbb{E}X^k$为$X$的$k$阶**原点矩**（记为$\mu_k$）
- 性质：
  - 若高阶矩存在，则低阶矩必存在
  - $\mu_1=\mathbb{E}X,\nu_2=\mathrm{Var}X$
...
- 数学期望定义：设$(X,Y)$为二维随机向量，若$\mathbb{E}X$与$\mathbb{E}Y$均存在（通过边际分布求得），则称$(\mathbb{E}X,\mathbb{E}Y)$为$(X,Y)$的数学期望（向量）【也可推广为$d$维随机向量，下略】
- 方差定义：类似一维随机变量，故省略
- 定理：设随机变量$Z=g(X,Y)$为$(X,Y)$的函数，若$\mathbb{E}Z$存在，则
  $$
...
- 性质（注：下面将$X,Y$替换为$f(X),g(Y)$也成立）：
  ...
  - 若$X,Y$相互独立，则$\mathbb{E}(XY)=\mathbb{E}X\cdot \mathbb{E}Y$
  - $\mathrm{Var}(X\pm Y)=\mathrm{Var}X+\mathrm{Var}Y\pm 2\mathbb{E}[(X-\mathbb{E}X)(Y-\mathbb{E}Y)]$
  - $\mathbb{E}[(X-\mathbb{E}X)(Y-\mathbb{E}Y)]=\mathbb{E}(XY)-\mathbb{E}X\cdot \mathbb{E}Y$
  - 若$X$与$Y$相互独立，则：
...
- 定义：设$(X,Y)$为二维随机变量，若$\mathbb{E}[(X-\mathbb{E}X)(Y-\mathbb{E}Y)]$存在，则称其为$X$与$Y$的协方差，记为$\operatorname*{Cov}(X,Y)$.
- 性质：
  - $\operatorname*{Cov}(X,Y)=\operatorname*{Cov}(Y,X)$
  - $\operatorname*{Cov}(aX,bY)=ab\operatorname*{Cov}(X,Y)$
  - $\operatorname*{Cov}(X,Y)=\mathbb{E}(XY)-\mathbb{E}X\cdot \mathbb{E}Y$（计算时更常用）
...
- 定义：若$(X,Y)$为二维随机向量，则称
  ...
  - 在实际计算中，一般采用下述公式：
    ...
    $$
    即计算$\mathbb{E}X,\mathbb{E}Y,\mathbb{E}X^2,\mathbb{E}Y^2,\mathbb{E}(XY)$五个量。
- 性质：
  - 若$X^*=\frac{X-\mathbb{E}X}{\sqrt{\mathrm{Var}X}},Y^*=\frac{Y-\mathbb{E}Y}{\sqrt{\mathrm{Var}Y}}$，则$\mathrm{Corr}(X,Y)=\mathrm{Cov}(X^*,Y^*)=\mathrm{Corr}(X^*,Y^*)=\mathbb{E}(X^*Y^*)$
...
- 设$\mathbb{E}(X\mid Y=y)=g(y)$，则可以考虑随机变量$Y$的函数$g(Y)$，记为$\mathbb{E}(X\mid Y)$（即$Y=y$时取值$\mathbb{E}(X\mid Y=y)$）。
- 重期望公式：
  - 设$(X,Y)$为二维随机变量，且$\mathbb{E}X$存在，则$\mathbb{E}X=\mathbb{E}(\mathbb{E}(X\mid Y))=\mathbb{E}(g(Y))$.
  - 本质为全概率公式：
    $$
    \mathbb{E}X=\left\{
      \begin{aligned}
...
- 条件方差公式：$\mathrm{Var}X=\mathbb{E}(\mathrm{Var}(X\mid Y))+\mathrm{Var}(\mathbb{E}(X\mid Y))$
  - 简单证明（基于重期望公式）：
    $$
    \begin{aligned}
...
1. 几乎处处收敛性质
   ...
   - 那么由引理可得：
     ...
     - 若$\{X_n,n\geq 1\}$为相互独立的随机变量序列，$c$为常数，则
       ...
       $$
2. 依概率收敛性质
   - 若随机变量序列$\{X_n,n\geq 1\}$满足$\lim_{n\to\infty}\mathbb{E}X_n^2=0$，则$X_n\stackrel{P}{\longrightarrow}0$（可使用马尔科夫不等式证明）
   ...
   - 判别：若$X_n\stackrel{P}{\longrightarrow}X$，则
     ...
     - $$
        lim_{n\to\infty}\mathbb{E}(\frac{|X_n-X|}{1+|X_n-X|})=0
       $$
3. 依分布收敛性质
...
- 前提概念：
  - 复值随机变量：设$X,Y$为$(\Omega,\mathcal{F},P)$上的实值随机变量，则称$Z=X+iY$为复值随机变量（若$\mathbb{E}X,\mathbb{E}Y$存在，则称$\mathbb{E}Z=\mathbb{E}X+i\mathbb{E}Y$为$Z$的数学期望）；
...
- 定义：设$X$为随机变量，其分布函数为$F$，则称复值函数
  $$
  f(t)=\mathbb{E}e^{itX}=\mathbb{E}[\cos(tX)+i\sin(tX)]=\mathbb{E}[\cos(tX)]+i\mathbb{E}[\sin(tX)]
  $$
  为$X$（或$F$）的特征函数。
...
5. 特征函数与矩的关系：若随机变量$X$的$k$阶矩$\mu_k=\mathbb{E}X^k$存在，则$\forall j,1\leq j\leq k,f(t)$的$j$阶导数存在，且
   $$
   f^{(j)}(0)=i^j\mu_j
   $$
...
- 定义：设$\mathbf{X}=(X_1,\cdots,X_d)^T$为$d$维随机变量，则称
  $$
  f(t_1,\cdots,t_d)=\mathbb{E}e^{i\mathbf{t}^T\mathbf{X}}=\mathbb{E}\operatorname*{exp}\{i\sum_{i=1}^dt_iX_i\}
  $$
  为$\mathbf{X}$的特征函数，这里$\mathbf{t}=(t_1,\cdots,t_d)^T\in\mathbb{R}^d$
- 例：设$(X,Y)$服从二维正态分布$N(0,1;0,1;\rho)$，则$(X,Y)$的特征函数为
  $$
  f(t_1,t_2)=\mathbb{E}\operatorname*{exp}[i(t_1X+t_2Y)]=\operatorname*{exp}\{-\frac{t_1^2+t_2^2+2\rho t_1t_2}{2}\}
  $$

### 矩母函数（特征函数的实数版本）

- 定义：设$X$为一随机变量，对于$t\in\mathbb{R}$使得$\mathbb{E}[e^{tX}]<\infty$，则称
  $$
  M_X(t)=\mathbb{E}[e^{tX}]
  $$
  为$X$的矩母函数。
...
- 若$\mathbf{X}=(X_1,\cdots,X_d)^T$为$d$维随机变量，则称
  $$
  M_X(\mathbf{t})=\mathbb{E}e^{\mathbf{t}^T\mathbf{X}}=\mathbb{E}\operatorname*{exp}\{\sum_{i=1}^dt_iX_i\}
  $$
  为$\mathbf{X}$的特征函数，这里$\mathbf{t}=(t_1,\cdots,t_d)^T\in\mathbb{R}^d$
- 性质：
  1. 若随机变量$X$与$Y$相互独立，则$M_{X+Y}(t)=M_X(t)M_Y(t)$（与特征函数相同）
  2. 与原点矩的关系（基于泰勒展开）：$M_X^{(n)}(t)=\mathbb{E}[X^ne^{tX}]\Longrightarrow M_X^{(n)}(0)=\mathbb{E}[X^n]=\mu_n$
  3. 与中心矩的关系：设$\mu=\mathbb{E}X,M_{X-\mu}(t)=\mathbb{E}[e^{t(X-\mu)}]=e^{-\mu}M_X(t)$，则
     $M_{X-\mu}^{(n)}(t)=\mathbb{E}[(X-\mu)^ne^{t(X-\mu)}]\Longrightarrow M_{X-\mu}^{(n)}(0)=\mathbb{E}[(X-\mu)^n]=\nu_n$
...
2. **（弱）大数定律**
   ...
   - 注1：如无特别说明，一般取$a_n=\mathbb{E}S_n,b_n=n$，即
     $$
     \frac{S_n-\mathbb{E}S_n}{n}\stackrel{P}{\longrightarrow}0
     $$
   - 注2：若$\{X_n\}$独立同分布，上式也等价于
     $$
     \lim_{n\to\infty}P(|\frac{\sum_{k=1}^nX_k}{n}-\mathbb{E}X_1|\geq\epsilon)=0
     $$
     （伯努利大数定律就是$X_k\sim b(1,p)$的情形）
3. 其他大数定律（均对于随机变量序列$\{X_n,n\geq 1\}$，并设$S_n=\sum_{k=1}^nX_k$，满足下述条件之一即满足（弱）大数定律）
   - **切比雪夫大数定律**：$\{X_n,n\geq 1\}$两两不相关，且$\{\mathrm{Var}X_n\}$一致有界；
   - **马尔科夫大数定律**：马尔科夫条件成立，即$\lim_{n\to\infty}\frac{\mathrm{Var}S_n}{n^2}=0$；
   - **辛钦大数定律**：$\{X_n,n\geq 1\}$独立同分布且$\mathbb{E}X_1$存在。

- 相互关系：
  - 伯努利大数定律是切比雪夫大数定律的特例；切比雪夫大数定律是马尔科夫大数定律的推论（前者成立，后者一定成立）；马尔科夫大数定律可以由切比雪夫不等式推得：
    $$
    P(|\frac{S_n-\mathbb{E}S_n}{n}|\geq \epsilon)=P(|S_n-\mathbb{E}S_n|\geq n\epsilon)\leq\frac{\mathrm{Var}S_n}{n^2\epsilon^2}\to 0(n\to\infty)
    $$
...
- 蒙特卡洛法计算定积分：
  ...
  &=\mathbb{E}(f(X))\hspace{1em}(X\sim U(0,1))\\
  &=\mathbb{E}Y\hspace{1em}(Y=F(X))
  \end{aligned}
  $$
...
- 设$\{X_n,n\geq 1\}$为相互独立的随机变量序列，对任意$n\geq 1$，记
  ...
  \right.
  $$
  另记$a_n=\sum_{k=1}^n\mathbb{E}Y_{n,k},S_n=\sum_{k=1}^nX_k$，若$n\to\infty$时，
  $$
  \begin{aligned}
  &\sum_{k=1}^nP(|X_k|>n)\to 0\\
  &\frac{1}{n^2}\sum_{k=1}^n\mathbb{E}Y_{n,k}^2\to 0
  \end{aligned}
  $$
  则有
  $$
...
- 定义：对于随机变量序列$\{X_n,n\geq 1\}$，若存在数列$\{a_n,n\geq 1\}$与$\{b_n,n\geq 1\}$（$\{b_n\}$单调增且趋于无穷），且有$n\to\infty$时，
  ...
  - 特别地，当$\{X_n,n\geq 1\}$独立同分布，且数学期望$\mathbb{E}X_1=\mu$存在，则有：$n\to\infty$时，
    $$
...
- 定义：设$\{X_n,n\geq 1\}$为随机变量序列，$S_n=\sum_{k=1}^n,n\geq 1$，若数学期望$\mathbb{E}S_n$与方差$\mathrm{Var}S_n$均存在，且
  $$
  S^*_n=\frac{S_n-\mathbb{E}S_n}{\sqrt{\mathrm{Var}S_n}}
  $$
  依分布收敛于标准正态分布$N(0,1)$，则称$\{X_n,n\geq 1\}$满足中心极限定理。
- 性质：若$\{X_n,n\geq 1\}$满足中心极限定理，则对任意$x\in\mathbb{R}$，
  $$
  \lim_{n\to\infty}P(\frac{S_n-\mathbb{E}S_n}{\sqrt{\mathrm{Var}S_n}}\leq x)=\Phi(x)
  $$
...
- **Lindeberg-Feller条件**：对于独立的随机变量序列$\{X_n,n\geq 1\}$，若$\mathbb{E}X_n=\mu_n,\mathrm{Var}X_n=\sigma_n^2<\infty$，记$s_n=\sqrt{\sum_{i=1}^n\sigma_i^2}$，若$\forall\epsilon>0$，
  $$
  \frac{1}{s_n^2}\sum_{i=1}^n \mathbb{E}(X_i-\mu_i)^2\cdot 1_{\{|X_i-\mu_i|>\epsilon s_n\}}\to 0\hspace{1em}(n\to\infty)
  $$
...
- **Lindeberg-Feller中心极限定理**：若独立随机变量序列$\{X_n,n\geq 1\}$满足Lindeberg-Feller条件，则对任意$x\in\mathbb{R}$，
  ...
  $$
- **Lyapunov定理**：对于独立的随机变量序列$\{X_n,n\geq 1\}$，$\mathbb{E}X_n=\mu_n,\mathrm{Var}X_n=\sigma_n^2<\infty$，记$s_n=\sqrt{\sum_{i=1}^n\sigma_i^2}$，若存在$\delta>0$，满足
  $$
